document.addEventListener('DOMContentLoaded', () => {
  ...
  downloadBtn.addEventListener('click', async () => {
    try {
      ...
      const margin = 40;
      const baseLineHeight = 18.2; // Base line height (Medium)
      const maxWidth = width - 2 * margin;
      const entrySpacing = 8; // Extra spacing between entries
      const separatorLineThickness = 0.5; // Subtle line thickness
      const separatorLineColor = rgb(0.5, 0.5, 0.5); // 50% gray for subtlety
      ...
      const drawText = (text, font, size, isBold = false, x = margin, color = rgb(0, 0, 0)) => {
        ...
        for (const line of lines) {
          ...
          for (const word of words) {
            ...
            if (textWidth > maxWidth) {
              checkPageOverflow(lineHeight); // Check before drawing
        ...
        yPosition -= 4 * fontScale; // Extra spacing after section, scaled
      };

      // Function to draw a separator line
      const drawSeparatorLine = () => {
        checkPageOverflow(separatorLineThickness + 16); // Check space for line + spacing
        page.drawLine({
        ...
        yPosition -= 16; // Fixed spacing after line
      };
      ...
      // Draw resume content with separator lines
      drawText(data.name, font, 20.8, true); // Base 16 * 1.3
      drawText(data.contact, font, 13); // Base 10 * 1.3
      drawText(data.profileSummary, font, 13);
      drawSeparatorLine(); // Separator after Personal Details

      drawText('Professional Summary', font, 15.6, true); // Base 12 * 1.3
      drawText(data.professionalSummary, font, 13);
      drawSeparatorLine(); // Separator after Professional Summary
      ...
      drawSeparatorLine(); // Separator after Skills
      ...
      data.workExperience.forEach((exp, index) => {
        ...
        if (index < data.workExperience.length - 1) {
          checkPageOverflow(entrySpacing); // Check before adding spacing
          yPosition -= entrySpacing; // Add spacing between entries
        }
      });
      drawSeparatorLine(); // Separator after Work Experience
      ...
      drawSeparatorLine(); // Separator after Extracurricular Activities
      ...
      drawSeparatorLine(); // Separator after Education
      ...
      drawSeparatorLine(); // Separator after Personal Projects

      drawText('Hobbies and Interests', font, 15.6, true);
      drawText(data.hobbies, font, 13);
      drawSeparatorLine(); // Separator after Hobbies
